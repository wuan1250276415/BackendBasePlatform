...
/**
 * 元转元最小值，转成元后，2位精度，四舍五入，配合统一后台校验
 *
 * @program: tellhowcloud
 * @author: HawkWang
 * @create: 2021-12-26 07:16
 **/
public class YuanMinConstraintValidator implements ConstraintValidator<YuanMin, BigDecimal> {
    ...
    @Override
    public void initialize(YuanMin constraintAnnotation) {
        try {
            ...
            this.minValue = bigDecimal.setScale(this.scale, roundingMode);
        } catch (NumberFormatException nfe) {
            throw new IllegalArgumentException(String.format("%s不是有效的元格式的数据。", constraintAnnotation.value()));
        }
